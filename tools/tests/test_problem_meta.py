...
class TestPolygonField(unittest.TestCase):
    """The optional `polygon` provenance block.

    Its presence is a decision, not a decoration: `uploading-to-polygon`
    reads it to tell "create a new Polygon problem" from "re-sync the one
    this package already has". Every way the block can be malformed
    therefore has to arrive as a `ProblemMetaError`, not as an `id` of
    `None` handed to an API call.
    """

    GOOD = {"id": 123456, "owner": "setter",
            "url": "https://polygon.codeforces.com/p/setter/flight"}

    def loaded(self, block):
        return load(write(dict(VALID, polygon=block)))

    def test_absent_block_reads_as_none(self):
        # The ordinary state of every package that has not been uploaded.
        self.assertNotIn("polygon", VALID)
        self.assertIsNone(load(write(VALID)).polygon)

    def test_a_valid_block_loads_onto_the_problem_object(self):
        ref = self.loaded(self.GOOD).polygon
        self.assertIsNotNone(ref)
        self.assertEqual(ref.id, 123456)
        self.assertEqual(ref.owner, "setter")
        self.assertEqual(ref.url, self.GOOD["url"])
        # Absent until a revision has actually been committed.
        self.assertIsNone(ref.committed_at)

    def test_committed_at_loads_when_present(self):
        ref = self.loaded(dict(self.GOOD, committed_at=1717171717)).polygon
        self.assertEqual(ref.committed_at, 1717171717)

    def test_rejects_a_non_positive_id(self):
        for bad_id in (0, -1, -123456):
            with self.subTest(id=bad_id):
                with self.assertRaisesRegex(ProblemMetaError, "polygon.id"):
                    self.loaded(dict(self.GOOD, id=bad_id))
    ...
    def test_rejects_a_missing_key(self):
        for key in ("id", "owner", "url"):
            with self.subTest(missing=key):
                ...
                del block[key]
                with self.assertRaisesRegex(ProblemMetaError, f"polygon.{key}"):
                    self.loaded(block)

    def test_rejects_wrong_types(self):
        for key, bad_value in (("id", "123456"), ("id", 1.5), ("id", True),
                               ("owner", 7), ("url", None),
                               ("committed_at", "1717171717")):
            with self.subTest(key=key, value=bad_value):
                with self.assertRaisesRegex(ProblemMetaError, f"polygon.{key}"):
                    self.loaded(dict(self.GOOD, **{key: bad_value}))

    def test_rejects_a_non_positive_committed_at(self):
        with self.assertRaisesRegex(ProblemMetaError, "polygon.committed_at"):
            self.loaded(dict(self.GOOD, committed_at=0))

    def test_rejects_a_block_that_is_not_an_object(self):
        for bad in (123456, "123456", [123456]):
            with self.subTest(value=bad):
                with self.assertRaisesRegex(ProblemMetaError, "polygon"):
                    self.loaded(bad)

    def test_rejects_explicit_null_rather_than_reading_it_as_absent(self):
        # The costly direction of R1: reading `null` as "not uploaded" would
        # send the upload skill off to create a *second* Polygon problem for
