...
4. **The phase sequence**, with the loop-back edges drawn — statement (no `\Examples`) → model solution → `preparing-tests` → `validating-solutions` → samples → back to `writing-statements`. A surviving rejected solution routes back to the generators; disagreeing accepted solutions route to the arbiter, and from there either to the model solution or out to `writing-statements` on ambiguity.
